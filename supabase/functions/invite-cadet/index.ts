...
Deno.serve(async request=>{
 ...
 const {data:existingProfile}=await userClient.from('profiles').select('id').eq('email',normalizedEmail).maybeSingle();
 let inviteResult=await admin.auth.admin.generateLink({type:existingProfile?'recovery':'invite',email:normalizedEmail,options:{redirectTo:'https://det607flagdetail.com/?invite=1',data:{full_name:String(fullName).trim()}}});
 ...
 if(inviteResult.error?.message.toLowerCase().includes('already registered'))inviteResult=await admin.auth.admin.generateLink({type:'recovery',email:normalizedEmail,options:{redirectTo:'https://det607flagdetail.com/?invite=1'}});
