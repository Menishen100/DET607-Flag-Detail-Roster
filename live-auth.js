...
function applyRoleAccess(profile){const staff=['ADMIN','SUPER_ADMIN'].includes(profile?.admin_level),superAdmin=profile?.admin_level==='SUPER_ADMIN';document.querySelector('#publish').hidden=!superAdmin;document.querySelector('#assign-cadet').hidden=!superAdmin;document.querySelector('#block-date').hidden=!staff;document.querySelector('#edit-times')?.parentElement&&(document.querySelector('#edit-times').parentElement.hidden=!staff);document.querySelector('[data-view="attendance"]').hidden=!staff;document.querySelector('[data-view="counseling"]').hidden=!staff;document.querySelector('#record-attendance').hidden=!staff;document.querySelector('#new-case').hidden=!staff;document.querySelector('#edit-important-contact').hidden=!superAdmin;}

async function loadImportantInformation(){const {data:settings,error}=await supabaseClient.rpc('get_portal_important_information');if(error)return;const setting=Array.isArray(settings)?settings[0]:settings;if(!setting)return;document.querySelector('#important-contact-name').textContent=setting.contact_name;const email=document.querySelector('#important-contact-email');email.textContent=setting.contact_email;email.href=`mailto:${setting.contact_email}`;const discord=document.querySelector('#important-contact-discord');discord.textContent=setting.contact_discord?` or on Discord (${setting.contact_discord})`:'';}

function openImportantContactEditor(){if(window.det607CurrentProfile?.admin_level!=='SUPER_ADMIN')return toast('Only the Super Admin can update this contact.');const name=document.querySelector('#important-contact-name').textContent,email=document.querySelector('#important-contact-email').textContent,discord=document.querySelector('#important-contact-discord').textContent.replace(/^ or on Discord \(|\)$/g,'');modal(`<p class="eyebrow">SUPER ADMIN SETTINGS</p><h2>Operations contact</h2><p class="muted">This contact is displayed to every cadet across the portal.</p><div class="form-row"><label>Contact name</label><input id="important-contact-name-input" value="${escapeRosterText(name)}"></div><div class="form-row"><label>Contact email</label><input id="important-contact-email-input" type="email" value="${escapeRosterText(email)}"></div><div class="form-row"><label>Discord handle <span class="muted">(optional)</span></label><input id="important-contact-discord-input" value="${escapeRosterText(discord)}"></div><div class="modal-actions"><button class="secondary" onclick="close()">Cancel</button><button class="primary" id="save-important-contact">Save contact</button></div>`);document.querySelector('#save-important-contact').onclick=async()=>{const button=document.querySelector('#save-important-contact');button.disabled=true;button.textContent='Saving…';const {error}=await supabaseClient.rpc('update_portal_important_information',{new_contact_name:document.querySelector('#important-contact-name-input').value.trim(),new_contact_email:document.querySelector('#important-contact-email-input').value.trim(),new_contact_discord:document.querySelector('#important-contact-discord-input').value.trim()||null});if(error){button.disabled=false;button.textContent='Save contact';return toast(error.message);}await loadImportantInformation();close();toast('Operations contact updated for all cadets.');};}
