...
(() => {
  ...
  async function saveNewCase(send) {
    ...
    const payload = {
      ...
      drafted_text: facts, status: send ? 'AWAITING_CADET_SIGNATURE' : 'DRAFT', sent_at: send ? new Date().toISOString() : null
    };
    const { data: created, error } = await window.det607Supabase.from('counseling_cases').insert(payload).select().single();
    if (error) return showToast(error.message);
    if (send) {
      const subject = 'DET 607 Flag Detail Counseling Requires Your Review';
      const html = `<p>A counseling record has been created for your flag-detail assignment.</p><p>Please sign in to <a href="https://det607flagdetail.com">DET 607 Flag Detail Management</a> to review and acknowledge it.</p>`;
      const { error: notificationError } = await window.det607Supabase.functions.invoke('send-notification', { body: { recipientId: assignment.cadet_id, subject, html, eventType: 'COUNSELING_SIGNATURE_REQUEST', entityType: 'counseling_case', entityId: created.id } });
      if (notificationError) showToast(`Counseling saved, but email could not be sent: ${notificationError.message}`);
    }
    closeModal();
    showToast(send ? 'Counseling sent to the cadet for signature.' : 'Counseling draft saved.');
    await loadCases();
  }
  ...
  function openCase(id) {
    ...
    } else openModal(`${details}${item.cadet_explanation ? `<p><strong>Cadet explanation:</strong><br>${escapeHtml(item.cadet_explanation)}</p>` : ''}<div class="modal-actions"><button class="secondary" onclick="document.querySelector('#modal').classList.remove('show')">Close</button></div>`);
  }
