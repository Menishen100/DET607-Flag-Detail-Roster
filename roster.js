...
function renderRosterDirectory() {
  ...
  rosterDirectory.innerHTML = rows.length ? `<div class="roster-table"><div class="roster-row roster-head"><span>Cadet</span><span>Classification / access</span><span>Class level</span><span>Contact</span><span>Status</span></div>${rows.map(row => `<div class="roster-row"><strong>${row.full_name}</strong><span class="tag">${profileLabel(row)}</span><span>${row.class_level || 'Not set'}</span><span><a href="mailto:${row.email}">${row.email}</a><br>${row.phone ? `<a href="tel:${row.phone}">${row.phone}</a>` : 'No phone'}</span><span class="status ${row.active ? 'attended' : 'no-show'}">${row.active ? 'Active' : 'Inactive'}</span>${canEdit ? `<button class="secondary roster-edit" data-id="${row.id}">Edit</button>` : ''}</div>`).join('')}</div>` : '<p class="muted">No cadets match these filters.</p>';
