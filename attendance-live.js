...
(() => {
  ...
  function render() {
    ...
    list.innerHTML = rows.map(row => {
      ...
      return `<article class="attendance-item"><strong>${escapeHtml(detailLabel(row))}</strong><span>${escapeHtml(row.cadet_name)}<br><small class="muted">${escapeHtml(row.cadet_type)} · ${escapeHtml(row.assignment_position === 'POC_LEAD' ? 'POC lead' : 'GMC')}</small></span><span>${row.self_checked_in_at ? `Checked in ${new Date(row.self_checked_in_at).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}` : 'No check-in yet'}</span><span class="status ${String(row.status).toLowerCase().replace('_','-')}">${escapeHtml(displayStatus(row.status))}</span>${action}</article>`;
    }).join('');
